...
def save_model_data_to_json(data, filename):
    with open(filename, 'w') as json_file:
        json.dump(data, json_file, indent=4)



def load_model_data_from_json(filename):
...
def save_model_data(request):
    ...
    save_model_data_to_json(data_to_save, 'datos.json')
    return JsonResponse({'message': 'Datos guardados en JSON'})


def load_model_data(request):
    loaded_data = load_model_data_from_json('datos.json')
